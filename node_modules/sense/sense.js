...
Sense.prototype.get = function (id, callback) {
    var self = this;
     request.get(
...
Datastream.prototype.addPoint = function (value, at, callback) {
    ...
    self._queue.enqueue({feed_id: self.id, value: value.toString()});
    ...
    if (self._queue.isFull()) {
        var points = [],
            ...
            url = self._cosm.server + '/events/';

        while (typeof value !== "undefined") {
            points.push(value);
            value = self._queue.dequeue();
        }
        request.post({url: url, json: {datapoints: points}, headers: {'sense_key': self._cosm.apiKey}}, callback);
    }
};
